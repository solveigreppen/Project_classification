...
for c = 1:Nclass
    ...
    try
    gmms2_diag{c,1} = fitgmdist(data, 2,'RegularizationValue',0.1,'CovarianceType','diagonal'); %legger til regulariztionvalue for å unngå feilmelding
    catch exeption
        disp ('Noe er feil') 
...
for c = 1:Nclass
    data = Fs((c-1)*Ntrain+1:c*Ntrain,:);
    try
    gmms3_diag{c,1} = fitgmdist(data, 3,'RegularizationValue',0.1,'CovarianceType','diagonal'); %legger til regulariztionvalue for å unngå feilmelding
    catch exeption
        disp ('Noe er feil') 
        error=exeption.message
    end
...
for c = 1:Nclass
    ...
    try
    gmms2{c,1} = fitgmdist(data, 2,'RegularizationValue',0.5); %legger til regularizationvalue for å unngå feilmelding
    catch exeption
        disp ('Noe er feil') 
        error=exeption.message
    end
...
for c = 1:Nclass
    data = Fs((c-1)*Ntrain+1:c*Ntrain,:);
    try
    gmms3{c,1} = fitgmdist(data, 3,'RegularizationValue',0.5); %legger til regulariztionvalue for å unngå feilmelding
    catch exeption
        disp ('Noe er feil') 
        error=exeption.message
    end
